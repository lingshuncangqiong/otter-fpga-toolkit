...
- **运行时**: VS Code API、Node.js 内置模块及随包附带的 Windows x64 Verible 解析器；不在运行时下载依赖
...
2. **风险匹配验证** — 修改 `extension.js` 后至少执行 `node --check extension.js`；修改 JSON 后解析对应文件；需要安装验证时把测试 VSIX 输出到仓库根目录，使用独立的 -test-<commit> 文件名
...
- 模块体声明改用 Verible CST 提取字段和作用域，统一 localparam/reg/wire/genvar 的声明头列；单维范围冒号对齐，多维与 unpacked 数组保留。
- 初值超过96字符时不撑宽其它声明的尾列；模块头、端口和例化继续使用已确认版式。
- Ctrl+L 使用 worker；CLI共用排版核心，保留BOM和换行符，解析失败不写入，输出检查token一致性。
- Windows x64 VSIX 自带固定解析器及完整第三方许可证。未覆盖结构保留并报告；不宣称完整SystemVerilog格式化。
- 验证：57项Node回归PASS，六份RTL参考解析/排版检查PASS。沿用用户对CST测试包的编辑器试用反馈；本轮没有新增VS Code UI实测或厂商RTL验证。
- Marketplace沿用手动上传流程，正式包为仓库根目录的 `otter-fpga-toolkit-2.1.19.vsix`。
...
- 用户确认本轮效果并授权按流程发布；版本从 2.1.16 更新到 2.1.17，包含 8edc154、bae4d0a 的排版修复。
- 同一 module 参数表/端口表分别跨空行及职责注释对齐，内部声明保持局部分组；缺失字段不预留空列，修正分隔符及末行注释偏移。
- 保留字符串中的注释标记、连续空格与转义内容，避免格式化改变参数/连接语义。
- 复用已通过的 51 项测试和 JS 语法检查；发布仅改版本与文档，另核对 manifest 与正式 VSIX 内容。编辑器安装及 Marketplace 上线不由打包或 Git push 代替证明。
- Marketplace 由用户手动上传正式 VSIX；不读取旧凭据或执行自动 publish。

## v2.1.16 改动记录

- 用户确认测试包并授权发布，版本升级为 `2.1.16`；Marketplace 由用户手动上传。
- 声明和例化按局部分组计算列宽，数组声明支持多组 packed/unpacked 维度与初始化表达式；限制超长字段对公共列宽的影响，减少多余空格。编辑器与 CLI 共用实现。
- 自动 lint 对 `.sv/.svh` 优先使用 xvlog，显式选择工具仍优先；诊断标注检查器来源。
- 验证：48 项自动测试 PASS；用户已确认测试包。Icarus 数组默认初始化误报已用最小示例复现，同例 xvlog -sv PASS；不代表整个用户工程通过验证。
...
- 原仓库基线 af5a76e：标量声明仍为缺失位宽/初始化保留空列；尾部边界长度下，最长无初值声明的分隔符还会多偏一列。改为仅保留当前组实际存在的字段，统一分隔符列，继续按局部分组计算。
- 修复字符串内的 `//` 被误作注释、例化字符串连续空格/制表符被改写的问题。识别字符串、转义引号和转义标识符后再分离行注释；混合块注释及跨行字符串等未支持行保持原样。
- 增加两组回归，覆盖缺失字段、分隔符边界、字符串内容保留和幂等性。清除 manifest 测试中过时的 2.1.15 固定值，检查版本格式，不修改 package.json。
- PASS：JavaScript 语法与全部 50 项测试；真实 Test/04 两个 RTL 的副本经过修复后只改变空白，二次检查无需修改。没有重跑硬件验证。
- 本次检查发现 VS Code/Cursor 的已安装目录均标为 2.1.15，但源码不同：前者已有局部分组，后者仍全文件计算列宽。已安装目录未改写，测试包用于统一验证；保持版本 2.1.16，未升级版本、安装或发布。

## 2026-09-10 接口区域对齐（已纳入 v2.1.17）

- 同一 ANSI module 头内的参数表与端口表分别作为完整列宽区域；空行、职责注释不再让端口参差，时钟/复位与数据端口共享名称及注释列。参数与端口独立计算，模块体和不同 module 保持局部边界。
- 复用 RTL parser 的模块头范围，不靠空行猜接口结束；选区仍只写入选中行，列宽参考完整区域，长字段上限与字符串保护沿用。
- 新回归覆盖分组端口、参数空行、末行注释、长参数隔离、不同 module、范围格式化和幂等性。全部 51 项测试 PASS，保持版本 2.1.16，未安装或发布。
