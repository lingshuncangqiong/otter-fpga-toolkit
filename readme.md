...
- 同一连续声明区域内，显式 `parameter`、`reg`、`wire` 星号分区（如 `/***************reg*******************/`）共用类型修饰符、位宽、范围冒号、名称和等号列，跨空行和注释仍然对齐；保留原分组。初始化值后的分号和注释列各区独立，长参数值不把所有 wire 行拖宽。空的 function/port/mechine 标记及 genvar 不打断共享列；其它代码、预处理或缩进变化结束区域，不跨 generate、过程块或 module 作用域。选区只改选中行，列宽参考完整区域；module 参数表和端口表仍分别计算。
