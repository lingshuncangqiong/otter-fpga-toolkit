...
- 数组声明分别识别名称前后的多组维度及初始化表达式；普通维度先压缩表达式空格，再对齐同组单维位宽的范围冒号：表达式靠左，补齐空格放在冒号前，例如 `[7                        :0]`。非零下界原值保留；多维维度保持紧凑，不把三目或 `package::name` 中的冒号当作范围分隔符。不改表达式或删除括号，保留单词和独立操作符之间必要的分隔；含字符串、转义标识符或宏的维度保留原文。
- 同一 ANSI module 头内，参数表、端口表分别统一列宽并沿用各自首项缩进，空行和分组注释不拆开它们。端口位宽、名称和初值按实际长度计算，名称、分隔符和注释对齐，允许较长单行。参数长度不影响端口。
- 星号分区标记（如 `/***************reg*******************/`）之间的连续声明区域共用类型修饰符、位宽、范围冒号、名称和等号列，跨空行和注释仍然对齐；按实际声明类型识别，不依赖分区名称，因此 `mechine` 中的状态常量也与后续 reg 对齐。保留原分组；初始化值后的分号和注释列各区独立，长参数值不把所有 wire 行拖宽。空分区及 genvar 不打断共享列；其它代码、预处理或缩进变化结束区域，不跨 generate、过程块或 module 作用域。选区只改选中行，列宽参考完整区域；module 参数表和端口表仍分别计算。
- 没有显式声明区标记时，内部声明和不同例化保持局部分组：空行、分区标题及其它代码划分组，普通注释不拆组。
- 非端口声明采用长行例外：普通字段按实际长度对齐；位宽或“名称＋数组后缀”超过 64 字符时，该行独立排版，不撑宽其它声明。初始化表达式超过 96 字符时，左侧仍按组对齐，分号和注释就近接在表达式后，不参与公共尾列计算。这些阈值仅控制填充，不截断、改写表达式或强制换行。
