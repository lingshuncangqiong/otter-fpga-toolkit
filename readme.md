...
- 同一 ANSI module 头内，参数表、端口表分别统一列宽并沿用各自首项缩进，空行和分组注释不拆开它们。端口位宽、名称和初值按实际长度计算，名称、分隔符和注释对齐；允许较长单行，不受旧的 32/40/24 字符上限影响，不强制换行。参数长度不影响端口。
- 显式 `/***************reg*******************/`、`/***************wire******************/` 声明区内，跨空行和说明注释统一列宽，原分组空行、注释保留。reg 与 wire 分区独立；遇到其它代码、预处理或缩进变化结束区域，不跨 generate 或 module 作用域对齐。选区仍只改选中行，列宽参考完整声明区。
- 没有显式声明区标记时，内部声明和不同例化保持局部分组：空行、分区标题及其它代码划分组，普通注释不拆组。非端口声明的位宽、名称与数组维度、初值仍最多贡献 32、40、24 字符的公共列宽，避免局部变量被少量超长字段撑宽。
