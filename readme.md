...
- 点击块上方“并排查看时序图”，或光标位于块内时按`Alt+W`，默认源码与波形左右并排；悬停只显示简短操作入口，不自动弹出大图。
- 需要更多空间时用“独立查看”移回源码所在编辑区域，再用“并排查看”恢复分栏；“查看全图”只调整图的缩放，不改变编辑区布局。点击信号名可定位相应端口或声明，找不到时明确提示，不跳到无关活动文件。
