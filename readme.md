...
- 同一 ANSI module 头内，参数表、端口表分别统一列宽并沿用各自首项缩进，空行和分组注释不拆开它们。时钟/复位与其他端口的名称、分隔符和注释列统一；参数长度不影响端口。
- `);` 之后的内部声明和不同例化保持局部分组：空行、分区标题及其它代码划分组，普通注释不拆组。不同 module 独立计算，长字段仍只有限参与公共列宽。
