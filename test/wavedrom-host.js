...
exports.run = async function() {
    ...
    assert.equal(vscode.window.tabGroups.all.length, groupsBefore, 'default preview must not create another editor group');
    ...
    await vscode.commands.executeCommand('otter-fpga-toolkit.previewWaveformPanel', doc.uri.toString(), 1);
    ...
    assert.ok(vscode.window.tabGroups.all.length>groupsBefore,'explicit parallel view must create a separate group');
